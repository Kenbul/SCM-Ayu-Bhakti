<template>
  <AppLayout title="Edit Barang Masuk">
...
const props = defineProps({
  incoming_good: Object,
  suppliers: Array,
  items: Array
})

const form = useForm({
  supplier_id: props.incoming_good.supplier_id,
  item_id: props.incoming_good.item_id,
  quantity: props.incoming_good.quantity,
  date_in: props.incoming_good.date_in
})

const submit = () => {
  form.put(`/incoming-goods/${props.incoming_good.id}`, {
    onSuccess: () => {
      Toast.fire({
        icon: 'success',
        title: 'Barang masuk berhasil diperbarui'
      })
    }
  }
...
// const submit = () => {
//   form.put(`/incoming-goods/${form.id}`, {
//     )
// }
